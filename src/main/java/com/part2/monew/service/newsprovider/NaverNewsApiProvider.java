...
public class NaverNewsApiProvider implements NewsProvider {
    ...
    public List<NewsArticleDto> fetchNews(ProviderConfig config, List<String> keywords) {
        ...
        log.info("네이버 API 뉴스 수집 시작 - 키워드 {}개: {}", keywords.size(), keywords);
        ...
        for (String keyword : keywords) {
            try {
                ...
                // 카테고리 기반 추가 필터링 적용
                List<NewsArticleDto> filteredNews = applyCategoryBasedFilter(keywordNews, keywords, keyword);
                
                allNews.addAll(filteredNews);
                log.info("네이버 API에서 키워드 '{}' 기사 {}건 수집 (카테고리 필터링 후: {}건)", keyword, keywordNews.size(), filteredNews.size());
    ...
    private List<NewsArticleDto> fetchNewsByKeyword(ProviderConfig config, String keyword) {
        try {
            ...
            String url = UriComponentsBuilder.fromHttpUrl(config.getApiUrl())
                    ...
                    .queryParam("display", config.getDefaultDisplay())
                    .queryParam("sort", config.getDefaultSort())
                    .build()
                    .toUriString();
    ...
    private List<NewsArticleDto> parseNaverApiResponse(String jsonResponse, String keyword) {
        ...
        try {
            ...
            for (JsonNode item : items) {
                NewsArticleDto news = NewsArticleDto.builder()
                        .providerName(getProviderKey())
                        ...
                        .author(null) // 네이버 API에서는 기자명 제공하지 않음
                        .guid(item.path("link").asText()) // 네이버 링크를 고유 식별자로 사용
                        .thumbnailUrl(null) 
                        .build();
    ...
    private List<NewsArticleDto> applyAdditionalKeywordFilter(List<NewsArticleDto> newsList, List<String> allKeywords) {
        return newsList.stream()
                .filter(news -> containsAnyKeyword(news, allKeywords))
                .toList();
    }
    ...
    private boolean containsAnyKeyword(NewsArticleDto news, List<String> keywords) {
        ...
        log.debug("네이버 API 기사 내용: {}", content.length() > 100 ? content.substring(0, 100) + "..." : content);
        
        // 제목이나 내용에 키워드 중 하나라도 포함되어 있으면 true
        for (String keyword : keywords) {
            if (title.contains(keyword) || content.contains(keyword)) {
                log.debug("네이버 API 키워드 '{}' 매칭됨! (제목: {}, 내용: {})", 
                    keyword, 
    ...
    private Timestamp parsePublishedDate(String pubDate) {
        try {
            // 네이버 API 날짜 형식: "Mon, 04 Nov 2024 10:30:00 +0900"
            DateTimeFormatter formatter = DateTimeFormatter.ofPattern("EEE, dd MMM yyyy HH:mm:ss Z");
            return Timestamp.valueOf(pubDate);
        } catch (Exception e) {
    ...
    private List<NewsArticleDto> applyCategoryBasedFilter(List<NewsArticleDto> articles, List<String> allKeywords, String searchKeyword) {
        List<NewsArticleDto> filteredArticles = new ArrayList<>();
        
        for (NewsArticleDto article : articles) {
            String title = article.getTitle() != null ? article.getTitle() : "";
            String content = article.getSummaryOrContent() != null ? article.getSummaryOrContent() : "";
            
            // 제목과 내용에서 카테고리 추론
            String inferredCategory = categoryKeywordService.inferCategoryFromContent(title, content);
            
            log.debug("=== 기사 분석 ===");
            log.debug("제목: {}", title);
            log.debug("내용: {}", content.length() > 100 ? content.substring(0, 100) + "..." : content);
            log.debug("추론된 카테고리: {}", inferredCategory);
            log.debug("검색 키워드: {}", searchKeyword);
            log.debug("전체 관심사 키워드: {}", allKeywords);
            ...
            if (title.toLowerCase().contains(searchKeyword.toLowerCase()) || 
                content.toLowerCase().contains(searchKeyword.toLowerCase())) {
                shouldInclude = true;
